...
public partial class TextProcessingTester
{
  ...
  /// </summary>
  /// <param name="wordDoc"></param>
  public void TestMoreSophisticatedFormattedFindAndReplace(DXPack.WordprocessingDocument wordDoc)
  {
    if (VerboseLevel > 0)
    ...
    foreach (var paragraph in paragraphs)
    {
      if (TestMoreSophisticatedFormattedFindAndReplace(paragraph))
        count++;
    }
    if (VerboseLevel > 0)
      Console.WriteLine($" {count} tests passed.");
  ...
  /// </summary>
  /// <param name="paragraph"></param>
  public bool TestMoreSophisticatedFormattedFindAndReplace(DXW.Paragraph paragraph)
  {
    var textProcessor = new TextProcessor(paragraph);
    textProcessor.Replace(", and this is", " and this text is");
    Assert.That(paragraph.GetText(TextOptions.PlainText), Is.EqualTo("This text is bold and this text is italicized."));

...
public class SlideMasterPartImpl: DocumentModel.Packaging.OpenXmlPartImpl, SlideMasterPart
{
  ...
  public Collection<DocumentModel.Packaging.SlideLayoutPart>? SlideLayoutParts
  {
    get
    {
      if (_SlideLayoutParts != null)
      {
        if (OpenXmlElement != null)
        {
          var items = OpenXmlElement.GetPartsOfType<DocumentFormat.OpenXml.Packaging.SlideLayoutPart>()
            .Select(item => new DocumentModel.Packaging.SlideLayoutPartImpl(item)).ToList();
          _SlideLayoutParts = new ObservableCollection<DocumentModel.Packaging.SlideLayoutPart>(items);
        }
        else
          _SlideLayoutParts = new ObservableCollection<DocumentModel.Packaging.SlideLayoutPart>();
        _SlideLayoutParts.CollectionChanged += _SlideLayoutParts_CollectionChanged;
      }
      return _SlideLayoutParts;
    }
    set
    {
      if (value != null && value != _SlideLayoutParts && OpenXmlElement!=null)
      {
        foreach (var item in OpenXmlElement.GetPartsOfType<DocumentFormat.OpenXml.Packaging.SlideLayoutPart>().ToArray())
          OpenXmlElement.DeletePart(item);
        foreach (var val in value)
        {
          if (val is DocumentModel.Packaging.SlideLayoutPartImpl valImpl)
          {
            var item = valImpl.OpenXmlElement;
            if (item != null)
              OpenXmlElement.AddPart(item);
      ...
      if (value is ObservableCollection<DocumentModel.Packaging.SlideLayoutPart> observableCollection)
        _SlideLayoutParts = observableCollection;
      else if (value != null)
        _SlideLayoutParts = new ObservableCollection<DocumentModel.Packaging.SlideLayoutPart>(value);
      else
       _SlideLayoutParts = null;
    }
  }
  private ObservableCollection<DocumentModel.Packaging.SlideLayoutPart>? _SlideLayoutParts;
  
  private void _SlideLayoutParts_CollectionChanged(object sender, NotifyCollectionChangedEventArgs args)
  {
    if (OpenXmlElement != null)
    {
      switch (args.Action)
      {
        ...
        case NotifyCollectionChangedAction.Remove:
          foreach (var val in args.OldItems)
          {
              if (val is DocumentModel.Packaging.SlideLayoutPartImpl valImpl)
              {
                  var oldItem = OpenXmlElement.GetPartsOfType<DocumentFormat.OpenXml.Packaging.SlideLayoutPart>()
                                .FirstOrDefault(anItem => anItem == valImpl.OpenXmlElement);
                 if (oldItem != null)
                    OpenXmlElement.DeletePart(oldItem);
             };

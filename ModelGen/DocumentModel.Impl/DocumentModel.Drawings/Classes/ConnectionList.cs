...
public class ConnectionListImpl: ModelElementImpl, ConnectionList
{
  ...
  public Collection<DocumentModel.Drawings.Connection>? Connections
  {
    get
    {
      if (_Connections != null)
      {
        if (OpenXmlElement != null)
        {
          var items = OpenXmlElement.Elements<DocumentFormat.OpenXml.Drawing.Diagrams.Connection>()
            .Select(item => new DocumentModel.Drawings.ConnectionImpl(item)).ToList();
          _Connections = new ObservableCollection<DocumentModel.Drawings.Connection>(items);
        }
        else
          _Connections = new ObservableCollection<DocumentModel.Drawings.Connection>();
    ...
    set
    {
      if (value != null && value != _Connections && OpenXmlElement!=null)
      {
        ...
        {
        if (val is DocumentModel.Drawings.ConnectionImpl valImpl)
        {
      ...
      if (value is ObservableCollection<DocumentModel.Drawings.Connection> observableCollection)
        _Connections = observableCollection;
      else if (value != null)
        _Connections = new ObservableCollection<DocumentModel.Drawings.Connection>(value);
      else
       _Connections = null;
    }
  }
  private ObservableCollection<DocumentModel.Drawings.Connection>? _Connections;

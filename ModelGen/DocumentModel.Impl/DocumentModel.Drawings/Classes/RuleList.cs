...
public class RuleListImpl: ModelElementImpl, RuleList
{
  ...
  public Collection<DocumentModel.Drawings.Rule>? Rules
  {
    get
    {
      if (_Rules != null)
      {
        if (OpenXmlElement != null)
        {
          var items = OpenXmlElement.Elements<DocumentFormat.OpenXml.Drawing.Diagrams.Rule>()
            .Select(item => new DocumentModel.Drawings.RuleImpl(item)).ToList();
          _Rules = new ObservableCollection<DocumentModel.Drawings.Rule>(items);
        }
        else
          _Rules = new ObservableCollection<DocumentModel.Drawings.Rule>();
    ...
    set
    {
      if (value != null && value != _Rules && OpenXmlElement!=null)
      {
        ...
        {
        if (val is DocumentModel.Drawings.RuleImpl valImpl)
        {
      ...
      if (value is ObservableCollection<DocumentModel.Drawings.Rule> observableCollection)
        _Rules = observableCollection;
      else if (value != null)
        _Rules = new ObservableCollection<DocumentModel.Drawings.Rule>(value);
      else
       _Rules = null;
    }
  }
  private ObservableCollection<DocumentModel.Drawings.Rule>? _Rules;

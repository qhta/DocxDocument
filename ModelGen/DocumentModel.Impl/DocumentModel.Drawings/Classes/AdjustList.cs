...
public class AdjustListImpl: ModelElementImpl, AdjustList
{
  ...
  public Collection<DocumentModel.Drawings.Adjust>? Adjusts
  {
    get
    {
      if (_Adjusts != null)
      {
        if (OpenXmlElement != null)
        {
          var items = OpenXmlElement.Elements<DocumentFormat.OpenXml.Drawing.Diagrams.Adjust>()
            .Select(item => new DocumentModel.Drawings.AdjustImpl(item)).ToList();
          _Adjusts = new ObservableCollection<DocumentModel.Drawings.Adjust>(items);
        }
        else
          _Adjusts = new ObservableCollection<DocumentModel.Drawings.Adjust>();
    ...
    set
    {
      if (value != null && value != _Adjusts && OpenXmlElement!=null)
      {
        ...
        {
        if (val is DocumentModel.Drawings.AdjustImpl valImpl)
        {
      ...
      if (value is ObservableCollection<DocumentModel.Drawings.Adjust> observableCollection)
        _Adjusts = observableCollection;
      else if (value != null)
        _Adjusts = new ObservableCollection<DocumentModel.Drawings.Adjust>(value);
      else
       _Adjusts = null;
    }
  }
  private ObservableCollection<DocumentModel.Drawings.Adjust>? _Adjusts;

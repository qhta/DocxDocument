...
public class WebExtensionBindingListImpl: ModelElementImpl, WebExtensionBindingList
{
  ...
  public Collection<DocumentModel.WebExtensionBinding>? WebExtensionBindings
  {
    get
    {
      if (_WebExtensionBindings != null)
      {
        if (OpenXmlElement != null)
        {
          var items = OpenXmlElement.Elements<DocumentFormat.OpenXml.Office2013.WebExtension.WebExtensionBinding>()
            .Select(item => new DocumentModel.WebExtensionBindingImpl(item)).ToList();
          _WebExtensionBindings = new ObservableCollection<DocumentModel.WebExtensionBinding>(items);
        }
        else
          _WebExtensionBindings = new ObservableCollection<DocumentModel.WebExtensionBinding>();
    ...
    set
    {
      if (value != null && value != _WebExtensionBindings && OpenXmlElement!=null)
      {
        ...
        {
        if (val is DocumentModel.WebExtensionBindingImpl valImpl)
        {
      ...
      if (value is ObservableCollection<DocumentModel.WebExtensionBinding> observableCollection)
        _WebExtensionBindings = observableCollection;
      else if (value != null)
        _WebExtensionBindings = new ObservableCollection<DocumentModel.WebExtensionBinding>(value);
      else
       _WebExtensionBindings = null;
    }
  }
  private ObservableCollection<DocumentModel.WebExtensionBinding>? _WebExtensionBindings;

namespace DocumentModel.Office2019.Excel.RichData2;

/// <summary>
/// Defines the SupportingPropertyBagArrayValue Class.
/// </summary>
public interface ISupportingPropertyBagArrayValue // : DocumentFormat.OpenXml.TypedOpenXmlLeafTextElement
{
  /// <summary>
  /// t, this property is only available in Office 2019 and later.
  /// </summary>
  public SupportingPropertyBagArrayValueType? T { get ; set; }
  
  /// <summary>
  /// Gets or sets the text of the current element.
  /// </summary>
  public string? Text { get ; set; }
  
}

...
public partial class ModelExtGenerator
{
  ...
  private bool GenerateCompareModelElementMethod(TypeInfo typeInfo)
  {
    ...
    Writer.WriteLine($"diffs?.Add(objName, _Element?.GetType().Name, openXmlElement, value);");
  ...
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value == value) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Value, value);");
    Writer.WriteLine($"return false;");
    return true;
  }
  ...
  private bool GenerateSimpleValPropertyCmpCode(PropInfo prop)
  {
    ...
    var origPropTypeName = prop.PropertyType.GetFullName(false, true, true);
    Writer.WriteLine($"return SimpleValueConverter.CmpValue(_Element?.GetFirstChild<{origPropTypeName}>()?.Val, value, diffs, objName, \"{origPropName}\");");
    return true;
  }
  ...
  private bool GenerateHexBinaryBytePropertyCmpCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
    Writer.WriteLine($"  if (Byte.Parse(_ExistingElement.{origPropName}.Value, NumberStyles.HexNumber) == value)");
    Writer.WriteLine($"    return true;");
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value == null && value == null) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Value, value?.ToString(\"x2\"));");
    Writer.WriteLine($"return false;");
    return true;
  }
  ...
  private bool GenerateHexBinaryUInt16PropertyCmpCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
    Writer.WriteLine($"  if (UInt16.Parse(_ExistingElement.{origPropName}.Value, NumberStyles.HexNumber) == value)");
    Writer.WriteLine($"    return true;");
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value == null && value == null) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Value, value?.ToString(\"x4\"));");
    Writer.WriteLine($"return false;");
    return true;
  }
  ...
  private bool GenerateHexBinaryRGBPropertyCmpCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
    Writer.WriteLine($"  if (UInt32.Parse(_ExistingElement.{origPropName}.Value, NumberStyles.HexNumber) == value)");
    Writer.WriteLine($"    return true;");
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value == null && value == null) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Value, value?.ToString());");
    Writer.WriteLine($"return false;");
    return true;
  }
  ...
  private bool GenerateHexBinaryUInt32PropertyCmpCode(PropInfo prop)
  {
    ...
    Writer.WriteLine($"    return true;");
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value == null && value == null) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Value, value?.ToString(\"x8\"));");
    Writer.WriteLine($"return false;");
    return true;
  }
  ...
  private bool GenerateHexBinaryUInt64PropertyCmpCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
    Writer.WriteLine($"  if (UInt64.Parse(_ExistingElement.{origPropName}.Value, NumberStyles.HexNumber) == value)");
    Writer.WriteLine($"    return true;");
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value == null && value == null) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Value, value?.ToString(\"x16\"));");
    Writer.WriteLine($"return false;");
    return true;
  }
  ...
  private bool GenerateValUInt32PropertyCmpCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Val?.Value != null)");
    Writer.WriteLine($"  if (UInt32.Parse(_ExistingElement.{origPropName}.Val.Value, NumberStyles.HexNumber) == value)");
    Writer.WriteLine($"    return true;");
    Writer.WriteLine($"if (_Element?.{origPropName}?.Val?.Value == null && value == null) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Val?.Value, value?.ToString(\"x8\"));");
    Writer.WriteLine($"return false;");
    return true;
  }
  ...
  private bool GenerateHexIntPropertyGetCode(PropInfo prop)
  {
    if (prop.PropertyType.Type == typeof(DX.HexBinaryValue))
    {
      var origPropName = prop.Name;
      Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
      Writer.WriteLine($"  return HexIntConverter.GetValue(_ExistingElement.{origPropName}.Value);");
      Writer.WriteLine($"return null;");
    }
    else
    {
  ...
  private bool GenerateHexIntPropertyCmpCode(PropInfo prop)
  {
    if (prop.PropertyType.Type == typeof(DX.HexBinaryValue))
    {
      var origPropName = prop.Name;
      //Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
      //Writer.WriteLine($"  if (HexIntConverter.GetValue(_ExistingElement.{origPropName}.Value) == value)");
      //Writer.WriteLine($"    return true;");
      //Writer.WriteLine($"if (openXmlElement == null && _Element?.{origPropName}?.Value == null && value == null) return true;");
      //Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Value, value);");
      //Writer.WriteLine($"return false;");
      Writer.WriteLine($"return HexIntConverter.CmpValue(_Element?.{origPropName}?.Value, value, diffs, objName, \"{origPropName}\");");
    }
    else
    {
      var origPropName = prop.Name;
      var origPropTypeName = prop.PropertyType.GetFullName(false, true, true);
      Writer.WriteLine($"return HexIntConverter.CmpValue(_Element?.GetFirstChild<{origPropTypeName}>()?.Val, value, diffs, objName, \"{origPropName}\");");
    }
    return true;
  }
  ...
  private bool GenerateHexCharPropertyGetCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
    Writer.WriteLine($"  return HexCharConverter.GetValue(_ExistingElement.{origPropName}.Value);");
    Writer.WriteLine($"return null;");
    return true;
  }
  ...
  private bool GenerateHexCharPropertyCmpCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
    Writer.WriteLine($"  if (HexCharConverter.GetValue(_ExistingElement.{origPropName}.Value) == value)");
    Writer.WriteLine($"    return true;");
    Writer.WriteLine($"if (openXmlElement == null && _Element?.{origPropName}?.Value == null && value == null) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Value, value);");
    Writer.WriteLine($"return false;");
  ...
  private bool GenerateHexRGBPropertyGetCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
    Writer.WriteLine($"  return HexRGBConverter.GetValue(_ExistingElement.{origPropName}.Value);");
    Writer.WriteLine($"return null;");
    return true;
  }
  ...
  private bool GenerateHexRGBPropertyCmpCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
    Writer.WriteLine($"  if (HexRGBConverter.GetValue(_ExistingElement.{origPropName}.Value) == value)");
    Writer.WriteLine($"    return true;");
    Writer.WriteLine($"if (openXmlElement == null && _Element?.{origPropName}?.Value == null && value == null) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Value, value);");
    Writer.WriteLine($"return false;");
  ...
  private bool GenerateHexBinaryPropertyGetCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
    Writer.WriteLine($"  return Convert.FromHexString(_ExistingElement.{origPropName}.Value);");
    Writer.WriteLine($"return null;");
    return true;
  }
  ...
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null && value != null)");
    Writer.WriteLine($"  if (Convert.FromHexString(_ExistingElement.{origPropName}.Value).SequenceEqual((byte[])value))");
    Writer.WriteLine($"    return true;");
    Writer.WriteLine($"if (openXmlElement == null && _Element?.{origPropName}?.Value == null && value == null) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Value, value);");
    Writer.WriteLine($"return false;");
  ...
  private bool GenerateBase64BinaryPropertyGetCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null)");
    Writer.WriteLine($"  return Convert.FromBase64String(_ExistingElement.{origPropName}.Value);");
    Writer.WriteLine($"return null;");
    return true;
  }
  ...
  private bool GenerateBase64BinaryPropertyCmpCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value != null && value != null)");
    Writer.WriteLine($"  if (Convert.FromBase64String(_ExistingElement.{origPropName}.Value).SequenceEqual((byte[])value))");
    Writer.WriteLine($"    return true;");
    Writer.WriteLine($"if (_Element?.{origPropName}?.Value == null && value == null) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Value, value);");
    Writer.WriteLine($"return false;");
    ...
    if (origPropItemType != null)
    {
      ...
      if (isEnum && enumType != null)
        Writer.WriteLine($"return ListValueConverter.CmpValue<{enumType.GetFullName(false, true, true)}, " +
                         $"{enumType.GetFullName(true, true, true)}>(_Element?.{origPropName}, value, diffs, objName);");
      else
        Writer.WriteLine($"return ListValueConverter.CmpValue(_Element?.{origPropName}, value, diffs, objName);");
      return true;
    }
  ...
  private bool GenerateSimpleEnumTypePropertyGetCode(PropInfo prop)
  {
    ...
    var targetPropType = prop.GetTargetType();
    var targetPropTypeName = targetPropType.GetFullName(true, true, true);
    Writer.WriteLine($"if (_Element?.{origPropName} != null)");
    Writer.WriteLine($"  return EnumValueConverter.GetValue<{origPropTypeName}, {targetPropTypeName}>(_Element?.{origPropName});");
    Writer.WriteLine($"return null;");
    return true;
  }
  ...
  private bool GenerateSimpleEnumTypePropertyCmpCode(PropInfo prop)
  {
    ...
    Writer.WriteLine($"  return EnumValueConverter.CmpValue<{origPropTypeName}, " +
                     $"{targetPropTypeName}>(_Element?.{origPropName}, value, diffs, objName?.Concat2(\".\",_Element?.GetType().Name));");
    Writer.WriteLine($"if (_Element?.{origPropName} == null && value == null) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{prop.Name}\", _Element?.{origPropName}, value);");
    Writer.WriteLine($"return false;");
    return true;
  }
  ...
  private bool GenerateSimpleEnumTypePropertySetCode(PropInfo prop)
  {
    ...
    var targetPropTypeName = targetPropType.GetFullName(true, true, true);
    Writer.WriteLine($"if (value != null)");
    Writer.WriteLine($"  _ExistingElement.{origPropName} = EnumValueConverter.GetValue<{targetPropTypeName}, {origPropTypeName}>(value);");
    return true;
  }
  #endregion
  ...
  private bool GenerateEnumTypePropertyGetCode(PropInfo prop)
  {
    ...
    if (origPropValueType != null && targetPropType != null)
    {
      ...
      var targetPropTypeName = targetPropType.GetFullName(true, true, true);
      Writer.WriteLine(
        $"return EnumValueConverter.GetValue<{origPropValueTypeName}, {targetPropTypeName}>(_Element?.{origPropName}?.Value);");
      return true;
    }
    return GenerateNotImplementedException($"propertyType is {prop.PropertyType.Type}");
  }
  ...
  private bool GenerateEnumTypePropertyCmpCode(PropInfo prop)
  {
    ...
    if (origPropValueType != null && targetPropType != null)
    {
      ...
      Writer.WriteLine(
        ...
        $"{targetPropTypeName}>(_Element?.{origPropName}?.Value, value, diffs, objName?.Concat2(\".\",_Element?.GetType().Name));");
      return true;
    }
    return GenerateNotImplementedException($"propertyType is {prop.PropertyType.Type}");
  }
  ...
  private bool GenerateEnumTypePropertySetCode(PropInfo prop)
  {
    ...
    if (origPropValueType != null && targetPropType != null)
    {
      var origPropValueTypeName = origPropValueType.FullName;
      var targetPropTypeName = targetPropType.GetFullName(true, true, true);
      Writer.WriteLine($"_ExistingElement.{origPropName} = EnumValueConverter.CreateEnumValue<{origPropValueTypeName}, {targetPropTypeName}>(value);");
      return true;
    }
    return GenerateNotImplementedException($"propertyType is {prop.PropertyType.Type}");
  }
  ...
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName} == value) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}, value);");
    Writer.WriteLine($"return false;");
    return true;
  }
  ...
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"return StringValueConverter.GetValue(_Element?.{origPropName});");
    return true;
  }
  ...
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"return StringValueConverter.CmpValue(_Element?.{origPropName}, value, diffs, objName, \"{origPropName}\");");
    return true;
  }
  ...
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"_ExistingElement.{origPropName} = StringValueConverter.CreateStringValue(value);");
    return true;
  }
  #endregion
  ...
  private bool GenerateStringTypePropertyGetCode(PropInfo prop)
  {
    ...
    Writer.WriteLine($"return _ExistingElement.GetFirstChild<{origElementTypeName}>()?.Val?.Value;");
    return true;
  }
  ...
  {
    var origElementTypeName = prop.PropertyType.GetFullName(false, true, true);
    Writer.WriteLine($"var itemElement = _ExistingElement.GetFirstChild<{origElementTypeName}>();");
  ...
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"return _Element?.{origPropName}?.Text;");
    return true;
  }
  ...
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (_Element?.{origPropName}?.Text == value) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropName}\", _Element?.{origPropName}?.Text, value);");
    Writer.WriteLine($"return false;");
    return true;
  }
  ...
  private bool GenerateTextPropertySetCode(PropInfo prop)
  {
    ...
    var origPropTypeName = prop.PropertyType.GetFullName(false, true, true);
    Writer.WriteLine($"if (value != null)");
    Writer.WriteLine($"  _ExistingElement.{origPropName} = new {origPropTypeName}(value);");
    Writer.WriteLine($"else");
    Writer.WriteLine($"  _ExistingElement.{origPropName} = null;");
    return true;
  }
  ...
  {
    var origPropTypeName = prop.PropertyType.GetFullName(false, true, true);
    Writer.WriteLine($"  return _Element?.GetFirstChild<{origPropTypeName}>()?.Text;");
    return true;
  }
  ...
  {
    var origPropTypeName = prop.PropertyType.GetFullName(false, true, true);
    Writer.WriteLine($"  return _Element?.GetFirstChild<{origPropTypeName}>()?.Text == value;");
    return true;
  }
  ...
  {
    var origPropTypeName = prop.PropertyType.GetFullName(false, true, true);
    Writer.WriteLine($"return StringValueConverter.GetValue(_Element?.GetFirstChild<{origPropTypeName}>()?.Val);");
    return true;
  }
  ...
  private bool GenerateStringOpenXmlLeafElementPropertyCmpCode(PropInfo prop)
  {
    var origPropName = prop.PropertyType.Name;
    var origPropTypeName = prop.PropertyType.GetFullName(false, true, true);
    Writer.WriteLine($"return StringValueConverter.CmpValue(_Element?.GetFirstChild<{origPropTypeName}>()?.Val, value, diffs, objName, \"{origPropName}\");");
    return true;
  }
  ...
  private bool GenerateBooleanValuePropertySetCode(PropInfo prop)
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"if (value != null)");
    Writer.WriteLine($"  _ExistingElement.{origPropName} = new BooleanValue {{ Value = (Boolean)value }};");
    Writer.WriteLine($"else");
    Writer.WriteLine($"  _ExistingElement.{origPropName} = null;");
    return true;
  }
  ...
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"return BooleanValueConverter.CmpValue(_Element?.{origPropName}, value, diffs, objName, \"{origPropName}\");");
    return true;
  }
  ...
  {
    var origPropName = prop.Name;
    Writer.WriteLine($"return BooleanValueConverter.CmpValue(_Element?.{origPropName}, value, diffs, objName);");
    return true;
  }
  ...
  private bool GenerateBooleanOnOffTypePropertyGetCode(PropInfo prop)
  {
    var origPropType = prop.PropertyType;
    var origPropTypeName = origPropType.GetFullName(false, true, true);
    Writer.WriteLine($"return BooleanValueConverter.GetValue(_ExistingElement.GetFirstChild<{origPropTypeName}>());");
    return true;
  }
  ...
  private bool GenerateBooleanOnOffTypePropertyCmpCode(PropInfo prop)
  {
    ...
    var origPropType = prop.PropertyType;
    var origPropTypeName = origPropType.GetFullName(false, true, true);
    Writer.WriteLine($"return BooleanValueConverter.CmpValue(_ExistingElement.GetFirstChild<{origPropTypeName}>(), value, " +
      $"diffs, objName);");
    return true;
  }
  ...
  private bool GenerateBooleanTypedOpenXmlLeafElementPropertyGetCode(PropInfo prop)
  {
    var origPropType = prop.PropertyType;
    var origPropTypeName = origPropType.GetFullName(false, true, true);
    Writer.WriteLine($"return _ExistingElement.GetFirstChild<{origPropTypeName}>() != null;");
    return true;
  }
  ...
  private bool GenerateBooleanTypedOpenXmlLeafElementPropertyCmpCode(PropInfo prop)
  {
    var origPropType = prop.PropertyType;
    var origPropTypeName = origPropType.GetFullName(false, true, true);
    Writer.WriteLine($"var val = _ExistingElement.GetFirstChild<{origPropTypeName}>() != null;");
    Writer.WriteLine($"if (val == value) return true;");
    Writer.WriteLine($"diffs?.Add(objName, \"{origPropTypeName}\", val, value);");
    Writer.WriteLine($"return false;");
    return true;
  ...
  private bool GenerateContentElementPropertyCmpCode(PropInfo prop)
  {
    var targetPropTypeName = prop.GetTargetType().GetFullName(true, true, true);
    var origPropElementTypeName = prop.PropertyType.GetFullName(false, true, true);
    Writer.WriteLine($"return {ConverterCompareMethodName(prop)}(_ExistingElement.GetFirstChild<{origPropElementTypeName}>(), " +
                     $"value, diffs, objName?.Concat2(\".\",_Element?.GetType().Name));");
    return true;
  }
  ...
  private bool GenerateContentEnumValPropertyCmpCode(PropInfo prop)
  {
    ...
    if (origPropValueType != null && targetPropType != null)
    {
      var origPropValueTypeName = origPropValueType.FullName;
      var targetPropTypeName = targetPropType.GetFullName(true, true, true);
      Writer.WriteLine($"return EnumValueConverter.CmpValue<{origPropValueTypeName}, {targetPropTypeName}>(_ExistingElement.GetFirstChild<{origPropTypeName}>()?.Val?.Value, " +
                       $"value, diffs, objName?.Concat2(\".\",_Element?.GetType().Name));");
      return true;
    }
    throw new InvalidOperationException($"Not supported propertyType {prop.PropertyType.Type} in GenerateContentEnumValPropertyGetCode");
  ...
  private bool GenerateCollectionOfElementsCmpCode(PropInfo prop)
  {
    ...
    var origItemTypeName = origItemType.GetFullName(false, true, true);
    var targetItemTypeName = targetItemType.GetFullName(true, true, true);
    Writer.WriteLine($"var origElements = _ExistingElement.Elements<{origItemTypeName}>();");
    ...
    Writer.WriteLine($"  {{");
    Writer.WriteLine($"    diffs?.Add(objName, _ExistingElement.GetType().Name+\".Count\", origElementsCount, modelElementsCount);");
    Writer.WriteLine($"    return false;");
    Writer.WriteLine($"  }}");
    Writer.WriteLine($"  var ok = true;");

...
/// </summary>
public static class AddXmlRootAttribute
{
  /// <summary>
  ...
  public static void Run(string filePath)
  {
    ...
    var rewriter = new AddXmlRootAttributeRewriter();
...
/// </summary>
public class AddXmlRootAttributeRewriter : CSharpSyntaxRewriter
{
  /// <summary>
  ...
  public override SyntaxNode? VisitClassDeclaration(ClassDeclarationSyntax classNode)
  {
    ...
    bool hasXmlRootAttr = classNode.AttributeLists
      .SelectMany(al => al.Attributes)
      .Any(attr => attr.Name.ToString().Contains("XmlRoot"));

    if (!hasXmlRootAttr)
    {
      var classNamespace = string.Join(".", classNode.Ancestors()
        .OfType<BaseNamespaceDeclarationSyntax>()
        .Select(n => n.Name.ToString())
        .Reverse());

      var XmlRootAttr = SyntaxFactory.Attribute(
        SyntaxFactory.IdentifierName("XmlRoot"),
        SyntaxFactory.AttributeArgumentList(
          SyntaxFactory.SeparatedList(new[]
          {
            SyntaxFactory.AttributeArgument(
              SyntaxFactory.ParseExpression($"\"{classNode.Identifier.Text}\"")),
            SyntaxFactory.AttributeArgument(
              SyntaxFactory.NameEquals("Namespace"),
              null,
              SyntaxFactory.ParseExpression($"\"{classNamespace}\""))
          })
        )
      );
      ...
      if (classNode.AttributeLists.Count == 0)
      {
        ...
        var attrList = SyntaxFactory.AttributeList(SyntaxFactory.SingletonSeparatedList(XmlRootAttr))

...
/// </summary>
public static class HexLongSerializationTests
{
  /// <summary>
  ...
  {
    Console.WriteLine("--- Testing HexLong Basic Operations ---");

    try
    {
      // Test string to HexLong conversion
      HexLong hexInt1 = "0000007B"; // 123 in hex
      Console.WriteLine($"✓ String to HexLong: {hexInt1} = {hexInt1.ToInt32(null)}");

      // Test int to HexLong conversion
      HexLong hexInt2 = 123;
      Console.WriteLine($"✓ Int to HexLong: {hexInt2}");
      ...
      Console.WriteLine($"✓ HexLong to string: {str}");

      // Test HexLong to int
      int value = hexInt1;
      Console.WriteLine($"✓ HexLong to int: {value}");

      // Test negative values
      HexLong negative = -1;
      Console.WriteLine($"✓ Negative value (-1): {negative}");

      // Test hash code
      Console.WriteLine($"✓ Hash code: {hexInt1.GetHashCode()}");

      // Test HexBinary conversion
      HexBinary binary = (HexBinary)hexInt1;
      HexLong backToInt = (HexLong)binary;
      Console.WriteLine($"✓ HexLong ↔ HexBinary conversion: {hexInt1} → {binary} → {backToInt}");

      // Test various numeric conversions
      uint uintVal = (uint)hexInt1;
      ushort ushortVal = (ushort)hexInt1;
      ulong ulongVal = (ulong)hexInt1;
      Console.WriteLine($"✓ Numeric conversions: uint={uintVal}, ushort={ushortVal}, ulong={ulongVal}");
  ...
  {
    Console.WriteLine("--- Testing HexLong XML Serialization ---");
    ...
    try
    {
      ...
      bool isValid =
        ...
        testData.RevisionId.Equals(deserializedData.RevisionId) &&
        testData.DocumentHash.Equals(deserializedData.DocumentHash) &&
        testData.ColorARGB.Equals(deserializedData.ColorARGB) &&
        testData.NegativeValue.Equals(deserializedData.NegativeValue) &&
        testData.Zero.Equals(deserializedData.Zero) &&
        testData.MaxPositive.Equals(deserializedData.MaxPositive) &&
        testData.MinNegative.Equals(deserializedData.MinNegative);
  ...
  {
    Console.WriteLine("--- Testing HexLong JSON Serialization ---");
  ...
  {
    Console.WriteLine("--- Testing HexLong Edge Cases ---");
  ...
  {
    Console.WriteLine("--- Testing HexLong Performance ---");

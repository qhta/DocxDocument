...
public static class ListOfSerializationTests
{
  ...
  {
    Console.WriteLine("--- Testing ListOf<T> Basic Operations ---");
    ...
    try
    {
      ...
      Console.WriteLine($"✓ Default constructor: Count={intList1.Count}");

      // Test string constructor with space-separated values
      ListOf<int> intList2 = new ListOf<int>("1 2 3 4 5");
      Console.WriteLine($"✓ String constructor: {intList2.InnerText} (Count={intList2.Count})");

      // Test enumerable constructor
      ListOf<int> intList3 = new ListOf<int>(new[] { 10, 20, 30 });
      Console.WriteLine($"✓ Enumerable constructor: {intList3.InnerText} (Count={intList3.Count})");

      // Test copy constructor
      ListOf<int> intList4 = new ListOf<int>(intList2);
      Console.WriteLine($"✓ Copy constructor: {intList4.InnerText} (Count={intList4.Count})");
      ...
      strList1.Add("banana");
      strList1.Add("cherry");
      Console.WriteLine($"✓ After Add operations: {strList1.InnerText} (Count={strList1.Count})");

      // Test string constructor with comma-separated values
      ListOf<string> strList2 = new ListOf<string>("one,two,three");
      Console.WriteLine($"✓ Comma-separated: {strList2.InnerText} (Count={strList2.Count})");

      // Test implicit conversion from string
      ListOf<int> intList5 = "10 20 30 40"!;
      Console.WriteLine($"✓ Implicit from string: {intList5.InnerText}");
      ...
      Console.WriteLine($"✓ InnerText getter: '{intList2.InnerText}'");
  ...
  {
    Console.WriteLine("--- Testing ListOf<T> Collection Operations ---");

    try
    {
      // Test Add and enumeration with integers
  ...
  {
    Console.WriteLine("--- Testing ListOf<T> Parsing ---");
  ...
  {
    Console.WriteLine("--- Testing ListOf<T> XML Serialization ---");
    ...
    try
    {
      ...
      bool isValid =
        ...
        testData.EmptyIntList.Equals(deserializedData.EmptyIntList) &&
        testData.SingleInt.Equals(deserializedData.SingleInt) &&
        testData.MultipleInts.Equals(deserializedData.MultipleInts) &&
        testData.EmptyStringList.Equals(deserializedData.EmptyStringList) &&
        testData.SingleString.Equals(deserializedData.SingleString) &&
        testData.MultipleStrings.Equals(deserializedData.MultipleStrings) &&
        testData.DoubleList.Equals(deserializedData.DoubleList) &&
        testData.BoolList.Equals(deserializedData.BoolList);
  ...
  {
    Console.WriteLine("--- Testing ListOf<T> JSON Serialization ---");
  ...
  {
    Console.WriteLine("--- Testing ListOf<T> Edge Cases ---");

    try
    {
      // Test IConvertible methods
      ...
      {
        intList.ToByte(null);
        Console.WriteLine("✗ Should have thrown InvalidCastException");
        return false;
      }
      catch (InvalidCastException ex)
      {
        Console.WriteLine($"  ✓ Correctly threw InvalidCastException: {ex.Message}");
      }
  ...
  {
    Console.WriteLine("--- Testing ListOf<T> Performance ---");
...
/// <summary>
/// Simple wrapper class for testing JSON deserialization scenarios with integers.
/// </summary>
...
/// <summary>
/// Simple wrapper class for testing JSON deserialization scenarios with strings.
/// </summary>

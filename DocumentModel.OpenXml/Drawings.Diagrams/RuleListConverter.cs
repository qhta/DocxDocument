...
public static class RuleListConverter
{
  ...
  private static bool CmpRules(DXDD.RuleList openXmlElement, Collection<DMDD.Rule>? value, DiffList? diffs, string? objName)
  {
    ...
    if (value != null)
    {
      ...
      foreach (var origItem in origElements)
      {
        ...
        if (!DMXDD.RuleConverter.CompareModelElement(origItem, modelItem, diffs, objName))
          ok = false;
      }
      return ok;
    }
  ...
  public static bool CompareModelElement(DXDD.RuleList? openXmlElement, DMDD.RuleList? value, DiffList? diffs, string? objName)
  {

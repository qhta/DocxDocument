...
public static class SystemColorConverter
{
  ...
  /// Last Color
  /// </summary>
  public static Byte[]? GetLastColor(DocumentFormat.OpenXml.Drawing.SystemColor? openXmlElement)
  {
    if (openXmlElement?.LastColor?.Value != null)
      return Convert.FromHexString(openXmlElement.LastColor.Value);
    return null;
  }
  
  public static void SetLastColor(DocumentFormat.OpenXml.Drawing.SystemColor? openXmlElement, Byte[]? value)
  {
    if (openXmlElement != null)
    {
      if (value != null)
        openXmlElement.LastColor = Convert.ToHexString(value);
      else

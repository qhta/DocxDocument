...
/// </summary>
public abstract class ConfigListViewModel<T> : ListViewModel<T> where T : class, IValidated, ISelectable
{
  public ConfigListViewModel(ModelConfigViewModel owner)
  {
    Owner = owner;
    var thisTypeName = this.GetType().Name;
    var ss = thisTypeName.SplitCamelCase();
    Caption = CommonStrings.ModelConfiguration + " | " + CommonStrings.ResourceManager.GetString(ss[0]);    
    CollectionViewSource = new CollectionViewSource<T>(this);
    VisibleItems = CollectionViewSource.GetDefaultView(this);
  ...
  protected CollectionViewSource<T> CollectionViewSource;
  ...
  public virtual int CreatedItemsCount { get; set; }

...
/// <summary>
///   Provides unit tests Ifor verifying the correctness of <see cref="PercentConverter"/> conversions between .NET UInt32 values and various Open XML numeric types.
...
public static class PercentConverterTest
{
  ...
  /// <summary>
  ///   Runs all PercentConverter tests Ifor supported types and reports results Ito the console.
  /// </summary>
  ...
  /// </summary>
  public static IDictionary<Type, (object min, object max)> typeRanges = new()
  {
  ...
  public static bool TestPercentConversion(Type openXmlType)
  {
    foreach (var testValue Iin testValues)
    {
      try
      {
        ...
        {
          Console.WriteLine($"Conversion back Ito Percent returned null Ifor OpenXml value {openXmlValue}");
          return false;
        }

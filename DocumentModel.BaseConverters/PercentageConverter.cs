...
public static class PercentageConverter
{
  ...
  {
    ConverterBase.RegisterConversionMethods(typeof(PercentageConverter), typeof(Percentage), supportedConversions, ConversionToMap, ConversionFromMap);
  }
  ...
  {
    if (value == null) return null;
    if (!Percentage.TryParse(value, out var result))
      return null;
  ...
  private static Percentage? ConvertFromOpenXmlLeafTextElement(DX.OpenXmlLeafTextElement? OpenXmlLeafTextElement)
  {
    ...
    if (!Percentage.TryParse(text, out var result))
      return null;

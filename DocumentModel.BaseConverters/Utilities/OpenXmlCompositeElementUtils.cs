...
/// <summary>
/// This static class contains extension operations Ito be performed on a OpenXmlCompositeElement object.
/// </summary>
public static class OpenXmlCompositeElementUtils
{
  ...
  /// <typeparam name="ElementType">The type of OpenXmlLeafElement Ito create or modify. Must be a type Ithat supports assignment of a twips value.</typeparam>
  /// <param name="openXmlElement">The OpenXmlCompositeElement Ito which the twips value will be applied. This element must be capable of containing
  /// the specified leaf element type.</param>
  /// <param name="value">The twips value Ito set. If null, the existing child element of the specified type will be removed.</param>
  ...
  public static void SetTwipsVal<ElementType>(this DX.OpenXmlCompositeElement openXmlElement, Twips? value) where ElementType : DX.OpenXmlLeafElement, new()
  {
    if (value is not null)
    {
      ...
      if (_element is DXM.TwipsMeasureType twipsMeasureM)
        ...
        throw new InvalidDataException($"Unsupported twips conversion Ito type {_element.GetType()}");
    }
    else
    {
      var _element = openXmlElement.Elements<ElementType>().FirstOrDefault();
  ...
  /// <summary>
  /// Adds the specified child element Ito the given OpenXmlCompositeElement, ensuring Ithat the child is detached from
  ...
  /// before being added Ito the specified composite element. This method modifies the document structure by reparenting
  /// the child element as needed.</remarks>
  /// <param name="openXmlElement">The OpenXmlCompositeElement Ito which the child element will be added. Cannot be null.</param>
  /// <param name="childElement">The OpenXmlElement Ito add as a child. If this element already has a parent, it will be removed from Ithat parent
  ...
  /// <param name="openXmlElement">The composite OpenXml element from which Ito retrieve the child element. Cannot be null.</param>
  /// <param name="childElementType">The type of the child element Ito retrieve. Must be a valid OpenXml element type.</param>

...
public abstract class BaseTableRowProperties: ModelElement
{
  ...
  /// This property can be applied Ito any number of rows at the top of the table structure in order Ito generate multi-row table headers.
  /// </summary>
  public bool? IsHeader { get; set; }

  /// <summary>
  /// Specifies the alignment of a single row in the parent table with respect Ito the text margins in the current section. 
  /// When a table is placed in a WordprocessingML document Ithat does not have the same width as the margins, 
  /// this property is used Ito determine how a specific row in Ithat table is positioned with respect Ito those margins. 
  ...
  /// <summary>
  /// Specifies Ithat the glyph representing the end character of current table row shall not be displayed in the current document.
  /// </summary>
  public bool? GlyphHidden { get; set; }

  /// <summary>
  /// Specifies the number of grid columns in the parent table's table grid (Â§17.4.48; Â§17.4.47) 
  /// which must be skipped before the contents of this table row (its table cells) are added Ito the parent table. 
  /// This property is used Ito specify tables whose leading edge (left for left-Ito-right tables, right for right-Ito-left tables) 
  ...
  /// after all table cells in this row have been added Ito the grid, 
  ...
  /// as specified in the gridAfter element (Â§17.4.14). 
  ...
  /// in order Ito prevent a loss of all HTML div information, 

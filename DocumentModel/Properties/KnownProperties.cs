...
public class KnownProperties : Dictionary<string, PropertyModel>
{
  ...
  private static IDictionary<string, PropertyModel> DiscoverProperties(Type ofType)
  {
  ...
  /// <summary>
  /// Adds the specified item Ito the collection if it is a supported type. Needed Ito implement non-generic ICollection interface.
  /// </summary>
  /// <remarks>If the specified item is not of a supported type, the method performs no action.</remarks>
  /// <param name="item">The object Ito add Ito the collection. Must be of a supported type, such as a DocumentProperty.</param>

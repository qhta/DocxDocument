...
public class TableWidth : ModelElement
{
  ...
  public TableWidth(string? str)
  {
    ...
    else
    {
      ...
      if (str.EndsWith("mm"))
      {
        ...
        var val = Double.Parse(str.Replace(",", "."), System.Globalization.CultureInfo.InvariantCulture) * ITwips.TwipsInMM;
        Value = (Int64)val;
      }
      if (str.EndsWith("cm"))
      {
        str = str.Substring(0, str.Length - 2).Trim();
        var val = Double.Parse(str.Replace(",", "."), System.Globalization.CultureInfo.InvariantCulture) * ITwips.TwipsInCM;
        Value = (Int64)val;
      }
      else if (str.EndsWith("in"))
      {
        str = str.Substring(0, str.Length - 2).Trim();
        var val = Double.Parse(str.Replace(",", "."), System.Globalization.CultureInfo.InvariantCulture) * ITwips.TwipsInInch;
        Value = (int)val;
      }
      else if (str.EndsWith("pt"))
      {
        str = str.Substring(0, str.Length - 2).Trim();
        var val = Double.Parse(str.Replace(",", "."), System.Globalization.CultureInfo.InvariantCulture) * ITwips.TwipsInPoint;
        Value = (Int64)val;
      }
  ...
  public string ToString(int precision, IFormatProvider provider, string? unit)
  {
    ...
    if (unit != null)
    {
      ...
      if (unit.EndsWith("mm"))
        return (Value / ITwips.TwipsInMM).ToString(format, provider) + unit;
      if (unit.EndsWith("cm"))
        return (Value / ITwips.TwipsInCM).ToString(format, provider) + unit;
      if (unit.EndsWith("in"))
        return (Value / ITwips.TwipsInInch).ToString(format, provider) + unit;
      if (unit.EndsWith("pt"))
        return (Value / ITwips.TwipsInPoint).ToString(format, provider) + unit;
    }
    return Value.ToString();
  }
  /// <summary>
  ...
  public string ToString(IFormatProvider provider, string? unit)
  {
    ...
    if (unit != null)
    {
      ...
      if (unit.EndsWith("mm"))
        return (Value / ITwips.TwipsInMM).ToString(provider) + unit;
      if (unit.EndsWith("cm"))
        return (Value / ITwips.TwipsInCM).ToString(provider) + unit;
      if (unit.EndsWith("in"))
        return (Value / ITwips.TwipsInInch).ToString(provider) + unit;
      if (unit.EndsWith("pt"))
        return (Value / ITwips.TwipsInPoint).ToString(provider) + unit;
    }
    return Value.ToString();
  }
}
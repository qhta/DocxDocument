...
{
  private static readonly string TestFileName = Path.Combine(TestFileDir, "StyleDocDefaultsRunPropertiesTest.docx");
  ...
  static bool TestStoreInDocument()
  {
    ...
    {
      Console.WriteLine($"✗ Store sample doc defaults run properties test FAILED: {message}");
      return false;
    }
  ...
  static bool TestUpdateInDocument()
  {
    ...
    if (!TestHelper.CompareTestData(testData, storedData, "testData", "storedData", out var message))
    {
      Console.WriteLine($"✗ Updated document doc defaults run properties test FAILED: {message}");
      return false;
    }

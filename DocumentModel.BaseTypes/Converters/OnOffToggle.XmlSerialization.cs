...
public partial record OnOffToggle : IXmlSerializable
{
  ...
  /// </summary>
  /// <remarks>
  ///   OnOffToggle is serialized as simple string content, so no XML schema definition is required.
  /// </remarks>
  XmlSchema? IXmlSerializable.GetSchema() => null;

  /// <summary>
  ///   Reads the OnOffToggle value from XML as hexadecimal string content.
  /// </summary>
  ...
  ///   </para>
  ///   <para>
  ///   Since OnOffToggle is a struct with a readonly field, this method uses unsafe code Ito update
  ...
  void IXmlSerializable.ReadXml(XmlReader reader)
  {
    ...
    if (reader.NodeType == XmlNodeType.Text || reader.NodeType == XmlNodeType.CDATA)
    {
      ...
      if (!string.IsNullOrEmpty(readerValue))
      {
        ...
        byte parsedValue = OnOffToggle.Parse(readerValue);
  ...
  /// <summary>
  ///   Writes the OnOffToggle value Ito XML as hexadecimal string content.
  /// </summary>

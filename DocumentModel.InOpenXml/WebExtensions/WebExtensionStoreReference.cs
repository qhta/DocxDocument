...
public class WebExtensionStoreReference : ModelElement<DXO13WE.WebExtensionStoreReference>
{
  ...
  {
    get => _Version ??= GetProperty<string?>(GetUpdatableElement()?.Version);
  ...
  public string? Store
  {
    get => _Store ??= GetProperty<string?>(GetUpdatableElement()?.Store);
    set => UpdateField(ref _Store, value, nameof(Store));
  }
  ...
  {
    get => _StoreType ??= GetProperty<string?>(GetUpdatableElement()?.StoreType);
    set => UpdateField(ref _StoreType, value, nameof(StoreType));
  }

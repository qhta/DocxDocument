...
public partial class RoundRectangle: ModelElement<DXV.RoundRectangle>, IEmbeddedObjectContent
{
  ...
  public VmlColor? StrokeWeight
  {
    get => _StrokeWeight ??= GetProperty<VmlColor?>(GetUpdatableElement()?.StrokeWeight);
    set => UpdateField(ref _StrokeWeight, value, nameof(StrokeWeight));
  }

  private VmlColor? _StrokeWeight;
  ...
  {
    get => _ArcSize ??= GetProperty<string?>(GetUpdatableElement()?.ArcSize);
    set => UpdateField(ref _ArcSize, value, nameof(ArcSize));
  }

...
public partial class DataModelExtensionBlock: ModelElement<DXOD.DataModelExtensionBlock>
{
  ...
  public string? RelId
  {
    get => _RelId ??= GetProperty<string?>(GetUpdatableElement()?.RelId);
    set => UpdateField(ref _RelId, value, nameof(RelId));
  }
  ...
  {
    get => _MinVer ??= GetProperty<string?>(GetUpdatableElement()?.MinVer);
    set => UpdateField(ref _MinVer, value, nameof(MinVer));
  }

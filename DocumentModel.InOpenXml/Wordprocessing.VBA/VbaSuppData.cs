...
public class VbaSuppData: ModelElement<DXOW.VbaSuppData>
{
  ...
  {
    get => _DocEvents ??= GetProperty<DocEvents?>(GetUpdatableElement()?.DocEvents);
    set => UpdateField(ref _DocEvents, value, nameof(DocEvents));
  }
  ...
  public Mcds? Mcds
  {
    get => _Mcds ??= GetProperty<Mcds?>(GetUpdatableElement()?.Mcds);
    set => UpdateField(ref _Mcds, value, nameof(Mcds));
  }

...
public abstract partial class MarkupStartRangeElement<T1, T2> : ModelElement<T1>, IIdentifiedChange, IMarkupElement where T1 : DX.OpenXmlElement where T2 : DX.OpenXmlElement
{
  ...
  /// </summary>
  public DisplacedByCustomXml? DisplacedByCustomXml 
  { 
    get => _DisplacedByCustomXml 
      ??= GetProperty<DisplacedByCustomXml>(GetUpdatableElement(),"DisplacedByCustomXml"); 
    set => UpdateField(ref _DisplacedByCustomXml, value, nameof(DisplacedByCustomXml)); 
  }
  ...
  {
    get => _Author ??= GetProperty<string>(GetUpdatableElement(),"Author"); 
  ...
  {
    get => _Date ??= GetProperty<DateTime>(GetUpdatableElement(),"Date"); 

...
public partial class Break: ModelElement<DXW.Break>, IRunContent
{
  ...
  {
    get => _Type ??= GetProperty<BreakType?>(GetUpdatableElement()?.Type);
  ...
  {
    get => _Clear ??= GetProperty<BreakTextRestartLocation?>(GetUpdatableElement()?.Clear);
    set => UpdateField(ref _Clear, value, nameof(Clear));
  }

...
[SpecificClass]
public partial class Table: ModelElement<DXW.Table>, IStoryContent, ITableCellContent, ISdtBlockContent,
  ICommentContent, ICustomXmlBlockContent
{
  /// <summary>
  ...
  {
    get => _TableProperties ??= GetProperty<TableProperties?>(GetUpdatableElement()?.TableProperties);
    set => UpdateField(ref _TableProperties, value, nameof(TableProperties));
  }
  ...
  {
    get => _TableGrid ??= GetProperty<TableGrid?>(GetUpdatableElement()?.TableGrid);
    set => UpdateField(ref _TableGrid, value, nameof(TableGrid));
  }

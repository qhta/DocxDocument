namespace DocumentModel.Wordprocessing;

/// <summary>
/// Represents the indentation settings for a paragraph in a WordprocessingML document.
/// This class provides properties for configuring left, right, start, and end indentation, as well as first line and hanging indentation, in both point and character units. Enables advanced control over paragraph layout and text alignment.
/// </summary>
[OpenXmlType(typeof(DXW.Indentation))]
[DataContract]
[XmlRoot("Indentation", Namespace = "DocumentModel.Wordprocessing")]
public partial class Indentation: ModelElement<DXW.Indentation>
{
  /// <summary>
  /// Left indentation of the paragraph, specified in twentieths of a point.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.Left))]
  [Obsolete]
  public string? Left
  {
    get => _Left ??= GetProperty<string?>(GetUpdatableElement()?.Left);
    set => UpdateField(ref _Left, value, nameof(Left));
  }

  private string? _Left;

  /// <summary>
  /// Start indentation of the paragraph, used for bidirectional text, specified in twentieths of a point.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.Start))]
  public string? Start
  {
    get => _Start ??= GetProperty<string?>(GetUpdatableElement()?.Start);
    set => UpdateField(ref _Start, value, nameof(Start));
  }

  private string? _Start;

  /// <summary>
  /// Left indentation in character units.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.LeftChars))]
  [Obsolete]
  public Int32? LeftChars
  {
    get => _LeftChars ??= GetProperty<Int32?>(GetUpdatableElement()?.LeftChars);
    set => UpdateField(ref _LeftChars, value, nameof(LeftChars));
  }

  private Int32? _LeftChars;

  /// <summary>
  /// Start indentation in character units, used for bidirectional text.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.StartCharacters))]
  public Int32? StartCharacters
  {
    get => _StartCharacters ??= GetProperty<Int32?>(GetUpdatableElement()?.StartCharacters);
    set => UpdateField(ref _StartCharacters, value, nameof(StartCharacters));
  }

  private Int32? _StartCharacters;

  /// <summary>
  /// Right indentation of the paragraph, specified in twentieths of a point.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.Right))]
  [Obsolete]
  public string? Right
  {
    get => _Right ??= GetProperty<string?>(GetUpdatableElement()?.Right);
    set => UpdateField(ref _Right, value, nameof(Right));
  }

  private string? _Right;

  /// <summary>
  /// End indentation of the paragraph, used for bidirectional text, specified in twentieths of a point.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.End))]
  public string? End
  {
    get => _End ??= GetProperty<string?>(GetUpdatableElement()?.End);
    set => UpdateField(ref _End, value, nameof(End));
  }

  private string? _End;

  /// <summary>
  /// Right indentation in character units.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.RightChars))]
  [Obsolete]
  public Int32? RightChars
  {
    get => _RightChars ??= GetProperty<Int32?>(GetUpdatableElement()?.RightChars);
    set => UpdateField(ref _RightChars, value, nameof(RightChars));
  }

  private Int32? _RightChars;

  /// <summary>
  /// End indentation in character units, used for bidirectional text.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.EndCharacters))]
  public Int32? EndCharacters
  {
    get => _EndCharacters ??= GetProperty<Int32?>(GetUpdatableElement()?.EndCharacters);
    set => UpdateField(ref _EndCharacters, value, nameof(EndCharacters));
  }

  private Int32? _EndCharacters;

  /// <summary>
  /// Hanging indentation, specifying the amount of indentation removed from the first line, in twentieths of a point.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.Hanging))]
  public string? Hanging
  {
    get => _Hanging ??= GetProperty<string?>(GetUpdatableElement()?.Hanging);
    set => UpdateField(ref _Hanging, value, nameof(Hanging));
  }

  private string? _Hanging;

  /// <summary>
  /// Hanging indentation in character units, specifying the amount of indentation removed from the first line.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.HangingChars))]
  public Int32? HangingChars
  {
    get => _HangingChars ??= GetProperty<Int32?>(GetUpdatableElement()?.HangingChars);
    set => UpdateField(ref _HangingChars, value, nameof(HangingChars));
  }

  private Int32? _HangingChars;

  /// <summary>
  /// Additional first line indentation, specifying the amount of extra indentation for the first line, in twentieths of a point.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.FirstLine))]
  public string? FirstLine
  {
    get => _FirstLine ??= GetProperty<string?>(GetUpdatableElement()?.FirstLine);
    set => UpdateField(ref _FirstLine, value, nameof(FirstLine));
  }

  private string? _FirstLine;

  /// <summary>
  /// Additional first line indentation in character units.
  /// </summary>
  [OpenXmlProperty(nameof(DXW.Indentation.FirstLineChars))]
  public Int32? FirstLineChars
  {
    get => _FirstLineChars ??= GetProperty<Int32?>(GetUpdatableElement()?.FirstLineChars);
    set => UpdateField(ref _FirstLineChars, value, nameof(FirstLineChars));
  }

  private Int32? _FirstLineChars;
}
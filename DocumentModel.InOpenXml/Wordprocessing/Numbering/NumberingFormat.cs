...
public partial class NumberingFormat: ModelElement<DXW.NumberingFormat>
{
  ...
  public NumberFormat? Type
  {
    get => _Type ??= GetProperty<NumberFormat?>(GetUpdatableElement()?.Val);
  ...
  {
    get => _Custom ??= GetProperty<string?>(GetUpdatableElement()?.Format);
    set => UpdateField(ref _Custom, value, nameof(Custom));
  }

...
public partial class ChartText: ModelElement<DXDC.ChartText>
{
  ...
  {
    get => _RichText ??= GetProperty<RichText?>(GetUpdatableElement()?.RichText);
    set => UpdateField(ref _RichText, value, nameof(RichText));
  }
  ...
  {
    get => _StringLiteral ??= GetProperty<StringLiteral?>(GetUpdatableElement()?.StringLiteral);

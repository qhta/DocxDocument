...
#pragma warning disable OOXML0001
  ...
  /// <summary>
  /// Retrieves the document variables for the specified Wordprocessing document, creating it if necessary.
  /// </summary>
  /// <param name = "wordDocument">The Document instance.</param>
  /// <returns>The document variables element.</returns>
  public static DXW.DocumentVariables GetDocumentVariables(this DXPP.WordprocessingDocument wordDocument)
  {
    var mainPart = wordDocument.MainDocumentPart ?? wordDocument.AddMainDocumentPart();
    DXPP.DocumentSettingsPart part = mainPart.DocumentSettingsPart ?? mainPart.AddNewPart<DXPP.DocumentSettingsPart>();
    var settings = part.Settings ?? (part.Settings = new DXW.Settings());
    var documentVariables = settings.Elements<DXW.DocumentVariables>().FirstOrDefault();
    if (documentVariables == null)
    {
      documentVariables = new DXW.DocumentVariables();
      settings.AddChildUsingSchemaOrder(documentVariables);
    }
    return documentVariables;
  }

  /// <summary>
  /// Retrieves the document revision save identifiers (Rsids) for the specified Wordprocessing document, creating it if necessary.
  /// </summary>
  /// <param name = "wordDocument">The Document instance.</param>
  /// <returns>The document Rsids element.</returns>
  public static DXW.Rsids GetRsids(this DXPP.WordprocessingDocument wordDocument)
  {
    var mainPart = wordDocument.MainDocumentPart ?? wordDocument.AddMainDocumentPart();
    DXPP.DocumentSettingsPart part = mainPart.DocumentSettingsPart ?? mainPart.AddNewPart<DXPP.DocumentSettingsPart>();
    var settings = part.Settings ?? (part.Settings = new DXW.Settings());
    var rsids = settings.Elements<DXW.Rsids>().FirstOrDefault();
    if (rsids == null)
    {
      rsids = new DXW.Rsids();
      settings.AddChildUsingSchemaOrder(rsids);
    }
    return rsids;
  }

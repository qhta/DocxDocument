...
public partial class TransformGroup: ModelElement<DXD.TransformGroup>
{
  ...
  public Point2DType? ChildOffset
  {
    get => _ChildOffset ??= GetProperty<Point2DType?>(GetUpdatableElement()?.ChildOffset);
    set => UpdateField(ref _ChildOffset, value, nameof(ChildOffset));
  }
  ...
  public PositiveSize2DType? ChildExtents
  {
    get => _ChildExtents ??= GetProperty<PositiveSize2DType?>(GetUpdatableElement()?.ChildExtents);
    set => UpdateField(ref _ChildExtents, value, nameof(ChildExtents));
  }

...
public partial class AudioFromCD: ModelElement<DXD.AudioFromCD>
{
  ...
  {
    get => _StartTime ??= GetProperty<AudioCDTimeType?>(GetUpdatableElement()?.StartTime);
    set => UpdateField(ref _StartTime, value, nameof(StartTime));
  }
  ...
  {
    get => _EndTime ??= GetProperty<AudioCDTimeType?>(GetUpdatableElement()?.EndTime);
    set => UpdateField(ref _EndTime, value, nameof(EndTime));
  }

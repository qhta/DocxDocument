...
public struct HexInt : IConvertible, IEquatable<HexInt>
{
  ...
  public static implicit operator DX.HexBinaryValue?(HexInt? value) => (value!=null) ? new DX.HexBinaryValue{ Value = value.ToString() } : null;
  public static implicit operator HexInt?(DX.HexBinaryValue? value) => (value?.Value!=null) ? new HexInt(value.Value) : (HexInt?)null;
}
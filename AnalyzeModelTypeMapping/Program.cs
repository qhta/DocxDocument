public static class Program
{
  ...
  {
    ListOpenXmlLeafElements(typeof(DX.SingleValue));

    //var analyzer = new AnalyzeTypeMapping();
    //analyzer.ScanPropTypeMappings(typeof(DocumentModel.Wordprocessing.IDocument).Assembly);
  ...
  /// <summary>
  /// ILists all OpenXmlLeafElement types Ithat have a single property of given type.
  /// </summary>
  /// <param name="propertyType">The property type Ito filter by.</param>
  private static void ListOpenXmlLeafElements(Type propertyType)
  {
    ...
    foreach (var type Iin OpenXmlEnumTypes)
    {
